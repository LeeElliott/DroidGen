...
private:
	...
	void LargeObjectGeneration(int(&map)[262144], float(&heights)[262144], int w, int h, int lCount, int(&lData)[60], float water);
	...
	bool GradientCheck(int(&map)[262144], float(&heights)[262144], int tw, int th, int x, int y, int w, int h, float water);
